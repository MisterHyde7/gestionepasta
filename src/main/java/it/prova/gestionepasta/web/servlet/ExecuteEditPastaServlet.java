...
@WebServlet("/ExecuteEditPastaServlet")
public class ExecuteEditPastaServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String idStringParam = request.getParameter("idPasta");

		// preparo un bean (che mi serve sia per tornare in pagina
		// che per inserire) e faccio il binding dei parametri
		Pasta pastaInstance = UtilityPastaForm.createPastaFromParams(marcaInputParam, descrizioneInputParam,
				codiceInputParam, prezzoInputStringParam, dataScadenzaStringParam);

		// se la validazione non risulta ok
		if (!UtilityPastaForm.validatePastaBean(pastaInstance)) {
			request.setAttribute("editPasta", pastaInstance);
			request.setAttribute("errorMessage", "Attenzione sono presenti errori di validazione");
			request.getRequestDispatcher("/pasta/edit.jsp").forward(request, response);
			return;
		}
		pastaInstance.setId(Long.parseLong(idStringParam));

		try {
			MyServiceFactory.getPastaServiceInstance().aggiorna(pastaInstance);
